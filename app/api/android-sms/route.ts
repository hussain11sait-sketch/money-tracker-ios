...
export async function POST(request: Request) {
  try {
    ...
    let targetUserId = body.user_id; // Grab the user ID if sent (e.g., from iOS Shortcut)
    ...
    // If targetUserId is missing (e.g., from Android background service), fetch all accounts globally to find a match
    let accountQuery = supabase.from('accounts').select('name, type, user_id');
    if (targetUserId) {
      ...
      for (const row of dbAccounts) {
        if (row.type === 'card' && row.name.includes('->')) {
          ...
          if (digitsMatch && lowerText.includes(digitsMatch[0])) {
            source = targetBank; 
            matched = true;
            if (!targetUserId) targetUserId = row.user_id; // Auto-capture user ID for Android
            break;
          }
        }
      }
    ...
    const { error: insertError } = await supabase.from('transactions').insert([{
      ...
      source: source,
      type: txType,
      user_id: targetUserId || null // Secures it to the user account
    }]);
